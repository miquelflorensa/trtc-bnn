"""
Experiment: Compare Analytical MM-Softmax vs Monte Carlo Ground Truth

This script trains a 3CNN on CIFAR-10 while tracking the accuracy of 
analytical moment approximations (MM-Softmax) compared to Monte Carlo sampling.
...
1. model_softmax: Full model with ClosedFormSoftmax (for training and getting probabilities)
2. model_logits: Same model without Softmax (for getting logit distributions)
...
def build_3cnn_softmax():
    """Build 3-block CNN with ClosedFormSoftmax head."""
    model = Sequential(
...
def build_3cnn_logits():
    """Build 3-block CNN WITHOUT Softmax (outputs logits)."""
    model = Sequential(
        Conv2d(3, 64, 3, bias=True, padding=1, in_width=32, in_height=32, 
               gain_weight=GAIN_W, gain_bias=GAIN_B),
        MixtureReLU(),
...
def compute_comparison_metrics(mu_z: np.ndarray, sigma_z: np.ndarray,
                                mu_softmax: np.ndarray, sigma_softmax: np.ndarray,
                                mc_samples: int = 10000):
    """
    Compare analytical MM-Softmax against MC-Softmax ground truth.
    
    Computes MAE for:
    - μ_A: Mean of Softmax output
    - σ_A: Std of Softmax output  
    - ρ_ZA: Correlation coefficient between input Z and output A
    
    Args:
        mu_z: Mean logits (B, K)
        sigma_z: Std of logits (B, K)
        mu_softmax: Mean probabilities from Softmax model (B, K) - for reference
        sigma_softmax: Std probabilities from Softmax model (B, K) - for reference
    ...
    # Compute MM-Softmax (analytical approximation)
    mm_results = mm_softmax(mu_z, sigma_z**2)
    mu_mm = mm_results['mu_a']
    sigma_mm = np.sqrt(mm_results['sigma_a_sq'])
    cov_z_a_mm = mm_results['cov_z_a']
    
    # Compute MC-Softmax (ground truth)
    mc_results = mc_softmax(mu_z, sigma_z**2, n_samples=mc_samples)
...
def synchronize_models(model_softmax: Sequential, model_logits: Sequential):
    """
    Synchronize weights from model_softmax to model_logits.
    Both models share the same architecture (except Softmax layer which has no weights).
    Uses state_dict/load_state_dict for weight transfer.
    """
    # Transfer parameters to host for state_dict access
    if model_softmax.device == "cuda":
        model_softmax.params_to_host()
    if model_logits.device == "cuda":
        model_logits.params_to_host()
    
    # Get state dict from softmax model and load into logits model
    state_dict = model_softmax.state_dict()
    model_logits.load_state_dict(state_dict)
    
    # Transfer back to device if using CUDA
    if model_softmax.device == "cuda":
        model_softmax.params_to_device()
    if model_logits.device == "cuda":
        model_logits.params_to_device()


def evaluate_epoch(model_softmax: Sequential, model_logits: Sequential,
    ...
    for images, labels in data_loader:
        ...
        m_prob, v_prob = model_softmax(batch_flat)
...
def train_epoch(model_softmax: Sequential, model_logits: Sequential,
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        # Forward pass on Softmax model (for training)
        m_pred, v_pred = model_softmax(batch_flat)
        ...
        # Update output layer
        out_updater.update(
            output_states=model_softmax.output_z_buffer,
            mu_obs=y,
            var_obs=var_y[:batch_size * 10],
            delta_states=model_softmax.input_delta_z_buffer,
        )
        
        # Backward pass and parameter update
        model_softmax.backward()
        model_softmax.step()
...
def run_experiment(epochs: int = EPOCHS, batch_size: int = BATCH_SIZE,
    ...
    """
    Run the full experiment comparing MM-Softmax vs MC-Softmax over training.
    """
    # Create directories
    ...
    print("="*70)
    print("Experiment: MM-Softmax vs MC-Softmax Comparison Over Training")
    print("="*70)
    ...
    for epoch in range(epochs):
        ...
        train_metrics = train_epoch(
            model_softmax, model_logits, out_updater, train_loader, var_y
        )
        ...
        val_metrics = evaluate_epoch(
            model_softmax, model_logits, val_loader, mc_samples=mc_samples
        )
        
        # Evaluate on test set
        test_metrics = evaluate_epoch(
            model_softmax, model_logits, test_loader, mc_samples=mc_samples
        )
    ...
    # Save results
    results_path = os.path.join(results_dir, "mm_softmax_comparison.npz")
    np.savez(results_path, **{k: np.array(v) for k, v in history.items()})
    print(f"\nResults saved to: {results_path}")
    
    # Save as JSON for easy inspection
    json_path = os.path.join(results_dir, "mm_softmax_comparison.json")
    with open(json_path, 'w') as f:
        json.dump({k: [float(x) for x in v] for k, v in history.items()}, f, indent=2)
    print(f"Results saved to: {json_path}")
    
    # Save final model
    model_path = os.path.join(save_dir, "3cnn_softmax_comparison.bin")
    if use_cuda:
        model_softmax.params_to_host()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare MM-Softmax vs MC-Softmax over training epochs"
    )
